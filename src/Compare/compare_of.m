...
if make_plot == 1
    ...
    for j = 1 : 3
        ...
        if uav_finite_size == 1
            for i = 1 : length(path_start)
                ...
                if j == 1
                    ...
                    plot(x,y1,'r','LineWidth',linewidth);
                elseif j == 2
                    
                    plot(x,y,'Color',[0,0.5,0],'LineWidth',linewidth); %g
                    plot(x,y1,'Color',[0,0.5,0],'LineWidth',linewidth); %g
                else
        ...
        elseif j == 2
            plot(Path_bez(:,1),Path_bez(:,2),'Color',[0,0.5,0],'LineWidth',linewidth); %g
            
        else
            plot(Path_bez(:,1),Path_bez(:,2),'b','LineWidth',linewidth); %b
    ...
    for j = 1 : n_obs
        ...
        x = obs(j,1) - obs_rad(j) : cs : obs(j,1)+ obs_rad(j);
    ...
    title('Red = Energy, Green = Time, Blue = Distance', 'FontSize', 14);
